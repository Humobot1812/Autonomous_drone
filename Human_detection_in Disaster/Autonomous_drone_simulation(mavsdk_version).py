...
yolo_model = YOLO("/home/raghuram/Documents/Nidar/Main/best _2.pt")  
...
ZIGZAG_SPACING = 64     # meters
CONNECTION_STRING = '/dev/ttyACM0'  # Serial connection for Cube Orange Plus

OFFBOARD_TIMEOUT = 40.0  # seconds

KML_FILE_PATH = f"/home/raghuram/Documents/Nidar/Main/survey_area.kml"  # Path to KML file
...
class VisionNode(Node):
    def __init__(self):
        ...
        self.local_object_present = False

        self.get_logger().info("📷 YOLO Vision Node started, waiting for /camera")
    ...
    def image_callback(self, msg):
            ...
            if cls == 0 and conf >= self.CONF_THRESH:
                ...
                with vision_lock:
                    ...
                    if not object_engaged:
                        ...
                        centered = abs(ex) < self.DEADZONE and abs(ey) < self.DEADZONE
                    else:
                        object_detected = False
                        centered = False

                self.last_seen_time = time.time()
                self.local_object_present = True
                found = True
        ...
        # # Lost target logic (unchanged)
        if not found and self.local_object_present and (time.time() - self.last_seen_time) > self.LOST_TIMEOUT:
            with vision_lock:
                object_detected = False
                centered = False
        ...
        # 🔵 SHOW WINDOW
        cv2.imshow("YOLO Vision (ROS2)", frame)
        cv2.waitKey(1)
    ...
    async def arm_and_takeoff(self, target_alt):
        ...
        async for position in self.drone.telemetry.position():
           ...
           if altitude >= target_alt * 0.95:
                ...
                print("🎥 Starting ROS2 Vision Node")

                threading.Thread(
                    target=start_ros_vision_node,
                    daemon=True
                ).start()
                break
